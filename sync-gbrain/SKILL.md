...
- `/sync-gbrain --full`：通过 `gbrain reindex-code` 做 full code reindex（大 repo 约 25-35 min）。**仅当 call graph 从未构建过时** 自动 build call graph（`gbrain dream`）。
- `/sync-gbrain --dream`：通过 source-scoped `gbrain dream --source <id>` cycle 构建此 source 的 call graph（`gbrain code-callers`/`code-callees`）；约数分钟；在 sync stages 后 lock-free 运行。即使已经 built 也 always forces。只有 code-aware schema pack 会产出 graph；否则 run 会报告 WARN，解释 graph 为什么仍为空。
- `/sync-gbrain --no-dream`：跳过 `--full` 原本会 auto-run 的 dream cycle。
...
`gbrain code-callers` / `code-callees`（who-calls-this / what-this-calls）会返回 `count: 0`，直到 `gbrain dream` cycle 为此 source 运行 `resolve_symbol_edges` phase。Step 2 的 code import 不会做这件事。

**One hard prerequisite：**构建 call graph 要求此 source active **schema pack extract code symbols**（`extract_atoms` phase）。不声明该能力的 pack（例如 `gbrain-base` / `gbrain-base-v2`）上，`dream` cycle 会完成，但 `resolve_symbol_edges` 什么都匹配不到；无论运行多少次，graph 都保持 empty。因此 "build the call graph" 只在 code-aware pack 上有意义。`--dream` stage 会检测并诚实报告（WARN row），而不是声称 build 了不存在的 graph。gbrain 只有在 cycle runtime 才暴露 pack capability（0.41.x 还没有 pre-flight query），所以无法在运行前检测。`code-def` / `code-refs` 需要相同的 symbol extraction；在 non-code-aware pack 上它们不是免费的 "direct lookups"。

通过 doctor 的 `cycle_freshness` check 检测此 source 的 call graph 是否 built，按 literal 匹配 cwd `SOURCE_ID`：
...
如果 `CYCLE == never`，且用户没有传 `--dream`/`--full`，且 Step 3 `PAGES > 0`，用 preamble 格式 AskUserQuestion：

> D2 — 此 repo 的 call graph 还没 built。现在 build 吗？
>
> ELI10：`gbrain code-callers`/`code-callees`（谁调用这个 function / 它调用什么）在此 source 的 `resolve_symbol_edges` phase 运行前会什么都不返回。`gbrain dream --source <this source>` 会运行它（scope 到此 worktree 的 code，耗时几分钟）。只有此 source 的 schema pack extracts code symbols 时才会产出 graph；如果不是，run 会完成但 graph 保持 empty，dream row 会明确说明。
>
> Recommendation：A — 在它运行前，call-graph queries 会返回 0，而且 code index 已经 populated。如果 A 返回 WARN（"pack does not extract code symbols"），修复方式是切换到 code-aware schema pack，而不是重复运行 dream。
>
> Note：options differ in kind, not coverage — no completeness score.
>
> A) 现在运行 /sync-gbrain --dream（recommended）
> B) 跳过，我稍后运行

如果选择 A：用 `--dream --code-only` 重新调用 orchestrator（跳过 memory + brain-sync；dream stage 仍会运行，因为它由 `--dream` gate）。然后报告 dream stage 的 ACTUAL row：`OK call graph built (N edges)`，或一个命名 graph 为什么仍 empty 的 `WARN`（non-code-aware pack、missing embedding key，或 0 edges matched）。WARN 时不要声称 success。
如果选择 B：记录 call-graph-not-built state，并继续 Step 4 的 verdict。

如果 `CYCLE == completed` 或 `unknown`，不要 prompt；但注意 `completed` 只表示有 cycle 运行过，不表示 edges 存在（non-code-aware pack 会在 empty graph 下报告 `completed`）。Step 5 的 verdict row 会浮出真实 state。
...
**此 worktree 通过 repo root 的 `.gbrain-source` file pin 到 worktree-scoped code source**（kubectl-style context）。在此 worktree 下任意位置调用 `gbrain code-def`、`code-refs`、`code-callers`、`code-callees`、`search` 或 `query`，默认都会 route 到该 source，不需要 `--source` flag（gbrain >= 0.41.38.0；更旧版本的 gbrain 对 call-graph commands 需要 `--source "$(cat .gbrain-source)"`）。同一个 repo 的 Conductor sibling worktrees 各自有自己的 pin 和 indexed pages，因此 semantic results 会匹配当前 worktree 磁盘上的实际 code。

Call-graph queries（`code-callers`/`code-callees`）还需要先 build graph；如果它们返回 `count: 0`，运行 `/sync-gbrain --dream`（或 `--full`）。这只有在此 source 的 gbrain schema pack extracts code symbols 时才有效；在 non-code-aware pack 上，`--dream` 会完成但 graph 保持 empty，并报告 WARN。`code-def`/`code-refs` 需要同样的 extraction。
...
**Call graph** row 报告可用的最权威 signal：

1. **如果此 invocation 运行了 dream stage**（`--dream`，或 `--full` auto-build），逐字 mirror 它的 row。这是本次 run 的 ground truth：
...
2. **否则** fallback 到 Step 3.5 的 `CYCLE` value，并使用诚实 wording（completed cycle 只证明 cycle 运行过，不证明 edges 存在）：
...
任何 `WARN` Call graph row 都会把 verdict 翻为 YELLOW。

如果任何 row 是 YELLOW 或 RED，verdict line 应说明，并在 failing rows 中给出一行 "next action"（例如 `Capability ...... ERR  capability check failed; CLAUDE.md guidance block REMOVED — run /setup-gbrain to repair`）。`never`/`unknown` Call graph row 会把 verdict 翻为 YELLOW。
