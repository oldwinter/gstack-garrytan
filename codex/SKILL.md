...
5a. **Synthesis recommendation（必需）。** 在呈现 Codex 原样输出和 GATE verdict 后，
...
3a. **Synthesis recommendation（必需）。** 在呈现完整 adversarial output 后，
输出一行 recommendation，总结用户下一步应该做什么；格式必须使用 AskUserQuestion judge 会评分的 canonical format：
...
8. **Synthesis recommendation（必需）。** 基于 Codex 的 consult output，
输出一行 recommendation，总结用户下一步应该做什么；格式必须使用 AskUserQuestion judge 会评分的 canonical format：
