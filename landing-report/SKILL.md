...
name: landing-report
version: 0.1.0
description: "workspace-aware ship 的 read-only queue dashboard。展示当前哪些 VERSION slots 已被 open PRs claim、哪些 sibling Conductor workspaces 有可能即将 ship 的 WIP work，"
triggers:
  - landing report
  - version queue
  - ship queue
...
以及 /ship 下一步会选择哪个 slot。无 mutations：只是 snapshot。
当用户要求 "landing report"、"what's in the queue"、"show me open PRs" 或
"which version do I claim next" 时使用。（gstack）
...
当你同时运行 5-10 个 parallel Conductor workspaces 时，一眼看清哪些 version numbers
被谁 claim、以及你的下一个 `/ship` 会落在哪个 slot，会非常有帮助。此 skill 是对
`/ship` 使用的同一个 `bin/gstack-next-version` utility 的 read-only call，但不会 mutate 任何东西。
可以把它理解成 VERSION numbers 的 `gh pr list`。

---

## Step 1：检测 platform 和 base branch

使用与其他 gstack skills 相同的 detection。
...
调用 util 三次：每个 bump level 一次，让用户看到 micro/patch/minor/major 分别会 claim 什么。
成本很低（相同 gh call 会被 bun cache）。
...
构建单个 table output。使用 `patch`-level JSON 作为 queue + siblings 的 canonical source
（它们在不同 bump levels 间相同；只有 `.version` 不同）。
...
★ active = VERSION ahead of base，且 last commit < 24h，且没有 open PR。
  这些 worktrees 最可能很快 ship。

如果现在运行 /ship，你会 claim：
...
对于 offline / unknown-host output，打印较短 block：
...
Fallback：local VERSION bumps 仍然可用，但无法检测 collisions。
```

---

## Step 5：建议 next action

Render table 后，建议以下其中一项：

1. **如果 queue 中有 collisions**（两个 open PRs claim 同一 version）：
...
2. **如果 active sibling outranks 用户的 branch version：**
...
PLAN MODE EXCEPTION：ALWAYS RUN。此 skill 完全 read-only：无 file writes、无 git mutations、
无 network state changes。可在 plan mode 中安全运行。
