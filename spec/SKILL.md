...
最后一行会把 spec 记录为 durable、issue-scoped cross-session decision，让未来 session（或 closing issue 的 `/ship`）继承 core approach 和 why，而不只是 issue link。Non-interactive、best-effort（`|| true`）。把 `ISSUE_NUMBER`（来自 filed issue）、`TITLE`（issue title）和 `APPROACH`（spec settled 的那个 core approach/decision）替换进去。只有 issue 实际创建后才运行。
