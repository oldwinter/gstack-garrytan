...
export const COMMAND_DESCRIPTIONS: Record<string, { category: string; description: string; usage?: string }> = {
  ...
  // Inspection
  'js':      { category: 'Inspection', description: '在 page context 中运行 inline JavaScript expression，并以 string 返回结果。与 eval 使用相同 JS sandbox；区别只是 js 接收 inline expr，而 eval 从 file 读取。带 --out <file> 时，结果写入 disk 而不是返回；base64 data URL 会自动解码为 raw bytes（除非传 --raw）。适合把本地 render rasterize 成 PNG，同时避免把大量 bytes 经 CLI 通道传回。--out 会让调用变成 WRITE（需要 write scope，且绝不允许通过 tunnel 使用）。', usage: 'js <expr> [--out <file>] [--raw]' },
  'eval':    { category: 'Inspection', description: '在 page context 中从 file 运行 JavaScript，并以 string 返回结果。Path 必须 resolve 到 /tmp 或 cwd 下（禁止 traversal）。multi-line scripts 用 eval；one-liners 用 js。带 --out <file> 时，结果写入 disk（base64 data URL 会解码为 bytes，除非传 --raw）；--out 会让调用变成 WRITE（需要 write scope，且绝不允许通过 tunnel 使用）。', usage: 'eval <file> [--out <file>] [--raw]' },
