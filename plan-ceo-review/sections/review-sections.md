...
* **One issue = one AskUserQuestion call（一个 issue = 一次 AskUserQuestion 调用）。** 绝不要把多个 issues 合并成一个 question。
...
第二个命令会把 accepted scope 记录为 durable cross-session decision，让下一个 session 不必重新争论就能看到已 settle 的内容和原因。它写入 `~/.gstack/`（与 review-log 同一 pattern），non-interactive，并且 best-effort（`|| true`，绝不阻塞 review）。替换 `SCOPE_SUMMARY`（例如 expansion 时 "accepted 4 of 6 proposals"，或 HOLD/REDUCTION 时 "held scope" / "cut 3 items"）和 `VERDICT`（summary 中的一行 verdict）。
